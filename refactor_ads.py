...
def refactor_html(file_path):
    ...
    if comment_index != -1 and ad_start_index - comment_index < 100:
    ...
    body_end_index = content.find(body_end_marker)
    ...
    # Let's find the last </div> before </body>
...
def main():
    files = [f for f in os.listdir('.') if f.endswith('.html') and f != '404.html']
    for file in files:
        refactor_html(file)
